...
class MembersScreen(Screen):
    ...
    class BackRequested(Message):
        """ Message to return to Data Product Catalogue screen """
        def __init__(self):
            super().__init__()
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            Button("Back", id="back"),
            Button("Details", id="details"),
            Button("Members", id="members"),
            Button("Quit", id="quit"),
            id="action_row",
        )
        yield Footer()
    ...
    async def back(self) -> None:
        self.log(f"Back button pressed")
        self.post_message(self.BackRequested())

    @on(Button.Pressed, "#details")
    async def details(self) -> None:
        self.log(f"Details button pressed")
        self.log(f"Posting Row Selected Message for App, Data: {self.selected_data}")
        self.post_message(self.MemberDetails(self.selected_data))

    @on(Button.Pressed, "#members")
    async def members(self) -> None:
        self.log(f"Members button pressed")
        self.log(f"Posting Row Selected Message for App, Data: {self.selected_data}")
        self.post_message(self.MemberSelected(self.selected_data))
