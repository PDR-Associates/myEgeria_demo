...
from demo_config import EgeriaConfig, get_global_config
from pyegeria import EgeriaTech


class DemoService(Widget):
    """ Service routines for data_products functions """
    def __init__(self):
        super().__init__()
    ...
    def find_collections(self) -> dict:
        if self._client and not self.token_expired():
    ...
    def egeria_login(self, payload) -> bool:
        self.payload = payload
        result = DemoService.access_egeria(self, payload=self.payload)
        return result
